/**
 * @file wirecomm.ino
 * @brief Wire communication example with an external device
 * Default wire address is 0x2B, change it in app_config_defs.h if needed.
 * @version 0.1
 * @date 2025-08-08
...
auto& mcuComm  = ynv::app::McuComm::getInstance();

void setup()
{
    displays.init(&appConfig);  // Initialize the ECD displays
    board.init(&appConfig);     // Initialize the board
    mcuComm.init(&appConfig);   // Initialize the MCU communication
    ...
    mcuComm.registerCallback(onCommandReceived);
}
...
void onCommandReceived(ynv::app::McuComm::Command_t data)
{
    Serial.println("Command received");
    // Handle the command as a button press
    onButtonPressed(static_cast<ynv::driverv5::Buttons::ButtonPress_t>(data));
}
